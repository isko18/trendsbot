...
async def show_buy_options(callback: CallbackQuery):
    ...
    await bot.send_message(
        ...
        text="💼 Выберите свой идеальный тариф и начните получать эксклюзивные тренды уже сегодня:",
        reply_markup=keyboard.as_markup()
    )
...
async def payment_done(message: Message):
    ...
    await message.answer(f"🎉 Готово! Вы открыли доступ к трендам.\n📅 Подписка действует до: <b>\n <b>{end.strftime('%Y-%m-%d %H:%M:%S')}</b></b>", parse_mode="HTML")
...
async def status(message: Message):
    ...
    if end and end > datetime.now():
        await message.answer(f"✨ Подписка подтверждена!\nВы с нами до: <b>{end.strftime('%Y-%m-%d %H:%M:%S')}</b>", reply_markup=InlineKeyboardBuilder().button(text="🔁 Продлить подписку", callback_data="open_buy_menu").adjust(1).as_markup(), parse_mode="HTML")
    else:
